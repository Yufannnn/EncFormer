...
from src.fhe.phantom.phantom_native_bench import run_score_full_native
from src.utils import print_stage


def main() -> None:
    p = argparse.ArgumentParser(description="Run native C++ CUDA Phantom Score full benchmark")
    ...
    print(
        f"[Score-nat] gpu={args.gpu} elapsed={out['elapsed_sec']:.3f}s "
        f"rel_err={out['rel_err']:.3e} binary={out['binary']}"
    )
