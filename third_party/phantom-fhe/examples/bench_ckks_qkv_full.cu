...
int main() {
    ...
    std::vector<double> A((size_t)M * (size_t)D1, 0.0);
    for (size_t i = 0; i < A.size(); i++) A[i] = nd(rng);

    std::vector<double> WQ((size_t)D1 * (size_t)D2, 0.0);
    std::vector<double> WK((size_t)D1 * (size_t)D2, 0.0);
    std::vector<double> WV((size_t)D1 * (size_t)D2, 0.0);
    ...
    std::vector<double> Q_ref = matmul_ref(A, M, D1, WQ, D2);
    std::vector<double> K_ref = matmul_ref(A, M, D1, WK, D2);
    std::vector<double> V_ref = matmul_ref(A, M, D1, WV, D2);

    std::vector<PhantomCiphertext> base_ct = encrypt_packed_pairs(ctx, encoder, sk, A, M, D1, HP, scale_in, C_USED_LIN);
    cuda_sync();
    ...
    KSCounters ks;
    auto babies = build_babies(ctx, gk, base_ct, N1, M, ks);

    std::vector<pc64> WQ_tab = build_wtab_paired(WQ, D1, D2, C, C_USED_LIN);
    std::vector<pc64> WK_tab = build_wtab_paired(WK, D1, D2, C, C_USED_LIN);
    std::vector<pc64> WV_tab = build_wtab_paired(WV, D1, D2, C, C_USED_LIN);

    const size_t ce = static_cast<size_t>(conj_elt);

    auto t0 = std::chrono::high_resolution_clock::now();
    auto Q_ct = linear_complex_paired(ctx, encoder, gk, babies, WQ_tab, D2, M, N1, N2, C, scale_w, ct_zero_mul, ks);
    auto K_ct = linear_complex_paired(ctx, encoder, gk, babies, WK_tab, D2, M, N1, N2, C, scale_w, ct_zero_mul, ks);
    auto V_ct = linear_complex_paired(ctx, encoder, gk, babies, WV_tab, D2, M, N1, N2, C, scale_w, ct_zero_mul, ks);
    ct_real_blocks(ctx, gk, Q_ct, ce, ks);
    ct_real_blocks(ctx, gk, K_ct, ce, ks);
    ct_real_blocks(ctx, gk, V_ct, ce, ks);
    cuda_sync();
    auto t1 = std::chrono::high_resolution_clock::now();

    std::vector<double> Q_got = decrypt_blocks(ctx, encoder, sk, Q_ct, M, D2);
    std::vector<double> K_got = decrypt_blocks(ctx, encoder, sk, K_ct, M, D2);
    std::vector<double> V_got = decrypt_blocks(ctx, encoder, sk, V_ct, M, D2);

    const double err_q = rel_err_real(Q_got, Q_ref);
    const double err_k = rel_err_real(K_got, K_ref);
    const double err_v = rel_err_real(V_got, V_ref);
    const double err_avg = (err_q + err_k + err_v) / 3.0;
    const double elapsed_ms = ms_since(t0, t1);

    std::cout << std::setprecision(10);
    std::cout << "stage qkv_full\n";
    ...
    std::cout << "rel_err " << std::scientific << err_avg << std::defaultfloat << "\n";
    std::cout << "ks_rots " << ks.rots << "\n";
    std::cout << "ks_muls_ctct " << ks.muls_ctct << "\n";
